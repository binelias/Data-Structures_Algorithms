...
class BinarySearchTree {
  ...
  BreadthFirstSearch(){
    ...
    let list = [];//answers
    let queue = [];//keep track level
    queue.push(currentNode);

    while(queue.length > 0){
      currentNode = queue.shift();//take first item in queue
      list.push(currentNode.value);//then move it to list
      if(currentNode.left) {
        queue.push(currentNode.left);//add to queue
      }
      if(currentNode.right) {
        queue.push(currentNode.right);//add to queue
      }
    }
    return list;
  }
...
//Trees-traversals-bfs-recursive
...
console.log('BFS', tree2.BreadthFirstSearch());
console.log('BFSR', tree2.BreadthFirstSearchR([tree2.root], []))
...
function traverse(node) {
  const tree2 = { value: node.value };
  tree2.left = node.left === null ? null : traverse(node.left);
  tree2.right = node.right === null ? null : traverse(node.right);
  return tree2;
}